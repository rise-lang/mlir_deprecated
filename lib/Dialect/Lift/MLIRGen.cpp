...
/// Implementation of a simple MLIR emission from the Lift AST.
///
/// This will emit operations that are specific to the Lift language, preserving
...
private:
  ...
  /// Helper conversion for a Lift AST location to an MLIR location.
  ...
  mlir::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    if (function.getNumArguments())
      function.setAttr("lift.generic", mlir::BoolAttr::get(true, &context));
  ...
  //   %0 = "lift.constant"() {value: dense<tensor<2x3xf64>,
  ...
  mlir::Value *mlirGen(CallExprAST &call) {
    ...
    if (callee == "transpose") {
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: lift.transpose "
  ...
  mlir::Value *mlirGen(ExprAST &expr) {
    switch (expr.getKind()) {
    case lift::ExprAST::Expr_BinOp:
      return mlirGen(cast<BinaryExprAST>(expr));
    case lift::ExprAST::Expr_Var:
      return mlirGen(cast<VariableExprAST>(expr));
    case lift::ExprAST::Expr_Literal:
      return mlirGen(cast<LiteralExprAST>(expr));
    case lift::ExprAST::Expr_Call:
      return mlirGen(cast<CallExprAST>(expr));
    case lift::ExprAST::Expr_Num:
  ...
  /// They are wrapped in a `lift` dialect (see next chapter) and get printed:
  ///   !lift.array<2, 2>
