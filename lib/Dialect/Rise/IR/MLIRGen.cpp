...
#include "mlir/Dialect/Rise/MLIRGen.h"
#include "mlir/Dialect/Rise/AST.h"
...
/// Implementation of a simple MLIR emission from the Rise AST.
///
/// This will emit operations that are specific to the Rise language, preserving
...
public:
  ...
  /// Public API: convert the AST for a Rise module (source file) to an MLIR
  /// Module.
...
private:
  ...
  /// Helper conversion for a Rise AST location to an MLIR location.
...
//      function.setAttr("rise.generic", mlir::BoolAttr::get(true, &context));
//
//    return function;
  }
...
//    switch (binop.getOp()) {
//    case '+':
//      return builder->create<mlir::rise::AddOp>(location, L, R).getResult();
//      break;
//    case '*':
//      return builder->create<mlir::rise::MulOp>(location, L, R).getResult();
//    default:
...
//      builder->create<mlir::rise::ReturnOp>(location);
//      return true;
//    }
//    auto *expr = mlirGen(*ret.getExpr().getValue());
//    if (!expr)
//      return false;
//    builder->create<mlir::rise::ReturnOp>(location, expr);
//    return true;
//  }

  // Emit a literal/constant array. It will be emitted as a flattened array of
  // data in an Attribute attached to a `rise.constant` operation.
  ...
  //   %0 = "rise.constant"() {value: dense<tensor<2x3xf64>,
...
//
//    // Build the MLIR op `rise.constant`, only boilerplate below.
//    return builder->create<mlir::rise::ConstantOp>(location, lit.getDims(), dataAttribute)
//        .getResult();
  }
...
//        emitError(location, "MLIR codegen encountered an error: rise.transpose "
//                            "does not accept multiple arguments");
//        return nullptr;
//      }
//      mlir::Value *arg = mlirGen(*call.getArgs()[0]);
//      return builder->create<mlir::rise::TransposeOp>(location, arg).getResult();
//    }
//
...
//    return builder->create<mlir::rise::GenericCallOp>(location, call.getCallee(), operands)
//        .getResult();
  }
...
//      return false;
//    auto location = loc(call.loc());
//    builder->create<mlir::rise::PrintOp>(location, arg);
//    return true;
  }
...
//                                            loc(num.loc()));
//    return builder->create<mlir::rise::ConstantOp>(location, attr).getResult();
  }

  // Dispatch codegen for the right expression subclass using RTTI.
  mlir::Value *mlirGen(ExprAST &expr) {
    switch (expr.getKind()) {
    case rise::ExprAST::Expr_BinOp:
      return mlirGen(cast<BinaryExprAST>(expr));
    case rise::ExprAST::Expr_Var:
      return mlirGen(cast<VariableExprAST>(expr));
    case rise::ExprAST::Expr_Literal:
      return mlirGen(cast<LiteralExprAST>(expr));
    case rise::ExprAST::Expr_Call:
      return mlirGen(cast<CallExprAST>(expr));
    case rise::ExprAST::Expr_Num:
...
//        value = builder
//                    ->create<mlir::rise::ReshapeOp>(
//                        location, value,
//                        getType(vardecl.getType()).cast<mlir::rise::RiseArrayType>())
//                    .getResult();
//      }
//    } else {
...
//  }
  ...
  /// They are wrapped in a `rise` dialect (see next chapter) and get printed:
  ///   !rise.array<2, 2>
//  template <typename T> mlir::Type getType(T shape) {
//    SmallVector<int64_t, 8> shape64(shape.begin(), shape.end());
//    return mlir::rise::RiseArrayType::get(&context, shape64);
//  }

  /// Build an MLIR type from a Rise AST variable type
